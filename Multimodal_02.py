#In Ollama there are two specific models that can work with images, this is where we code them
import streamlit as st
import requests

#modules for handling images
import base64
...
def main():
    ...
    with col_1.popover("Model Management",help="Manage Tools here"):
        if not available_models:
            ...
            if st.button(f"Download {model_to_download}"):
                try:
                 ...
                 if missing_models:
                      model_to_download = st.selectbox(
                          ":green[**✉️ Downlaod Model**]",list(missing_models)
                      )
                 ...
                 selected_model = st.selectbox(
                     ":red[**⛔ DELETE MODEL**]",available_models
                 )                    
                 if st.button(f"Delete **_{selected_model}_**",type="primary"):
                     try:
                         ...
                         st.error(
                             f""""Failed to delte model:{selected_model}. Error: {str(e)}""",
                             icon="😒",
                         ) 
                         ...
                         if response.status_code == 200:
                             ...
                             if llava_response:
                                 st.markdown(llava_response)
                                  ...
                                  st.error(
                                      f"""No response received from{selected_model}.""",
                                      icon="😒",
                                  )                                        
                                   
                         else:
                             st.error(
                                 f"""Falied to get a response from{selected_model}.""",
                                 icon="😒",
                             )   
